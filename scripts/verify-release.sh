...
repo_dir, repository, receipt_name, head = sys.argv[1:]
...
if (
    ...
    raise SystemExit("verify-release: Q04B receipt is not bound to HEAD")
print(receipt["archiveSha256"])
PY
)
metadata=$(gh -R "$repository" release view "$tag" --json tagName,isDraft,targetCommitish,assets)
python3 - "$metadata" "$tag" "$head" "$archive_name" "$checksum_name" "$expected_digest" <<'PY'
import json
import sys

value = json.loads(sys.argv[1])
if value.get("tagName") != sys.argv[2] or value.get("isDraft") is not False or value.get("targetCommitish") != sys.argv[3]:
    raise SystemExit("verify-release: published release metadata does not match HEAD")
...
archive = next(item for item in assets if item.get("name") == sys.argv[4])
if archive.get("digest") != "sha256:" + sys.argv[6]:
    raise SystemExit("verify-release: published archive digest does not match Q04B")
PY
...
archive_digest=$(python3 - "$download_dir/$archive_name" "$download_dir/$checksum_name" "$expected_digest" <<'PY'
import hashlib
import re
import stat
import sys
